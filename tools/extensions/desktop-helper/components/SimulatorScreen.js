...
XPCOMUtils.defineLazyModuleGetter(this, "GlobalSimulatorScreen",
                                  "resource://desktop-helper.js/modules/GlobalSimulatorScreen.jsm");

function debug() {
  //dump("SimulatorScreen.js: " + Array.slice(arguments) + "\n");
}
  ...
  _initChild: function(aWindow) {
    ...
    this._chromeObject = {
      ...
      mozLockOrientation: function(orientation) {
        // XXX When Gaia run in the browser the system app is included inside a <browser>
        // element and does not run as an application.
        let isSystemApp = /:\/\/system.gaiamobile.org/.test(aWindow.location.toString());
        if ((!isSystemApp && nodePrincipal.appStatus == nodePrincipal.APP_STATUS_NOT_INSTALLED) &&
