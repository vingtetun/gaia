...
//    PaginationBar.update(currentPage, pages.length);
  ...
  function initApps(apps) {
    ...
    appMgr.getAll().onsuccess = function onsuccess(event) {
      ...
      apps.forEach(function eachApp(app) {
        console.log('------ APP CREATED: ', app.manifestURL);
  ...
  function render(app) {
       ...
       * </li>
       */
       var target = document.getElementById('landing-page');
       console.log('------ app', app.manifest.name, app.title, app.icon);
  ...
  function processApp(app, callback) {
    console.log('---app processed', app.manifestURL);
    // Ignore system apps.
