...
function debug() {
  //dump("GlobalSimulatorScreen.js: " + Array.slice(arguments) + "\n");
}
...
this.GlobalSimulatorScreen = {
  ...
  lock: function() {
    ...
    let data = JSON.stringify({ locked: true });
    Services.obs.notifyObservers(null, "simulator-orientation-lock-change", data);
  },

  unlock: function() {
    GlobalSimulatorScreen.mozOrientationLocked = false;

    let data = JSON.stringify({ locked: false });
    Services.obs.notifyObservers(null, "simulator-orientation-lock-change", data);
  },
  ...
  adjustWindowSize: function() {
    ...
    let data = JSON.stringify({
      width: GlobalSimulatorScreen.width,
      height: GlobalSimulatorScreen.height
    });
    Services.obs.notifyObservers(null, "simulator-adjust-window-size", data);
  }
}
